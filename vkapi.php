...
if (!class_exists('VK_api')) :
    ...
    class VK_api
    {
        ...
        function __construct()
        {
            ...
            add_action('admin_head', array(&$this, 'post_notice')); # fix admin notice
            ...
            if ($option == 'true') {
                ...
                add_action(
                    'wp_ajax_update_vkapi_user_meta',
                    array(&$this, 'update_vkapi_user_meta')
                ); # update_vkapi_user_meta
            }
            ...
            add_action('wp_footer', array(&$this, 'vkapi_body_fix')); # body fix
            add_filter('the_content', array(&$this, 'add_buttons'), 88); # buttons
            add_filter(
                'plugin_action_links_' . plugin_basename(__FILE__),
                array(&$this, 'own_actions_links')
            ); # plugin links
                ...
                add_action(
                    ...
                    create_function(
                        ...
                        "echo '<div class=\"error\"><p>" . sprintf(
                            __('VKontakte API Plugin needs <a href="%s">configuration</a>.', self::$plugin_domain),
...
##### JS ASYNC LOADING
        ...
        <script charset="utf-8"
                type="text/javascript">if (window.Ya && window.Ya.Share) {
            Ya.Share.update();
        } else {
            (function () {
                if (!window.Ya) {
...
// https://www.ukraine.com.ua/tools/hosting/new_check_domains.php?name=darx&z[37]=biz.ua
        function add_head()
        {
            // VK API
        ...
        function create_menu()
        {
            ...
            self::$vkapi_page_settings = add_submenu_page(
                'vkapi_settings',
                'VKontakte API - ' . __('Settings', self::$plugin_domain),
            ...
            self::$vkapi_page_comments = add_submenu_page(
                'vkapi_settings',
                'VKontakte API - ' . __('Comments', self::$plugin_domain),
        ...
        function add_css()
        {
            ...
            #wp_register_script ( 'userapi', 'https://vk.com/js/api/openapi.js' );
            add_action('vkapi_body', array(&$this, 'js_async_openapi'));
        }
        ...
        function add_tabs_button_vk()
        {
            ...
            echo '
			<td>
			<button style="white-space:nowrap" class="submit" class="vk_recount" onclick="showVK()" >' . $vkapi_button . $vkapi_comm_show . '</button>
			</td>
			';
        }
        ...
        function add_tabs_button_fb()
        {
            echo '
			<td>
			<button style="white-space:nowrap" class="submit" onclick="showFB()" >' . __(
                'Facebook',
                self::$plugin_domain
            ) . ' (<fb:comments-count href="' . get_permalink() . '"></fb:comments-count>)</button>
			</td>
			';
        }
        ...
        function add_tabs_button_wp()
        {
            ...
            echo '
			<td>
			<button style="white-space:nowrap" class="submit" onclick="showWP()">' . __(
                'Site',
                self::$plugin_domain
            ) . ' (' . $comm_wp . ') </button>
			</td>
        ...
        function add_tabs()
        {
            if (comments_open()) {
                ...
                if ($show_comm == 'true' && ($vkapi_get_comm == '1' || !isset($vkapi_get_comm))) {
        ...
        function add_vk_comments()
        {
			...
			function vkapi_checkPermission() {
				if(window.webkitNotifications.checkPermission()==0){
					window.webkitNotifications.createNotification(
						"https://vk.com/images/lnkinner32.gif", "Успех",
			...
			function onChange(num,last_comment,data,hash){
				if (window.webkitNotifications.checkPermission() == 0) {
					...
					"https://vk.com/images/lnkinner32.gif", "Время "+Hour+":"+Min+":"+Sec,
					last_comment);
					notification.show();
					document.getElementById(\'vkapi_sound\').play();
					setTimeout(function(){notification.cancel();}, \'10000\');
				} else {
					jQuery("#vkapi").append(\'<button class="submit" class="vkapi_remove" onclick="vkapi_requestPermission()">Разрешить всплывающие сообщения</button>\');
				};
			};
			</script>';
            } else {
			...
			<script type="text/javascript">
				function VK_Widgets_Comments() {
					if ( typeof VK !== "undefined" )
						VK.Widgets.Comments(\'vkapi\', {width: ' . get_option(
                'vkapi_comm_width'
            ) . ', limit: ' . get_option(
                'vkapi_comm_limit'
            ) . ', attach: ' . $attach . ', autoPublish: ' . $autoPublish . ', height: ' . get_option(
                'vkapi_comm_height'
            ) . ', mini:1, pageUrl: "' . $url . '"},' . $post->ID . ');
					else setTimeout(VK_Widgets_Comments,1000);
				};
				VK_Widgets_Comments();
			</script>';
        ...
        {
            echo '
			<div style="background:white" class="fb-comments" data-href="' . get_permalink(
            ) . '" data-num-posts="' . get_option('vkapi_comm_limit') . '" data-width="' . get_option(
                'vkapi_comm_width'
...
########## start social buttons
        function add_buttons($args)
        {
            ...
            if (!is_feed() && !(in_array(
            ...
            )) && ($vkapi_get_butt == '1' || !isset($vkapi_get_butt))
            ) {
                ...
                add_action('wp_footer', array(&$this, 'social_button_style'), 88);
        ...
        function vkapi_button_like()
        {
            global $post;
            $postid = $post->ID;
            $divid = "vkapi_like_{$postid}_" . mt_rand();
            echo "<li><div id='$divid'></div></li>";
            ...
            echo "
						<script type=\"text/javascript\">
							<!--
								function VK_Widgets_Like_{$divid}() {
									if ( typeof VK !== 'undefined' ) {
										...
										setTimeout(VK_Widgets_Like_{$divid},1000);
									};
								};
								VK_Widgets_Like_{$divid}();
							-->
						</script>";
        }
        ...
        function vkapi_button_share()
        {
            global $post;
            $postid = $post->ID;
            $divid = "vkapi_share_{$postid}_" . mt_rand();
            echo "<li><div class='vkapishare' id='$divid'></div></li>";
            $vkapi_url = get_permalink();
            $vkapi_title = addslashes(do_shortcode($post->post_title));
            $vkapi_descr = str_replace(array("\r\n", "\n", "\r"), ' <br />', do_shortcode($post->post_content));
								...
								<!--
									function VK_Share_button_$divid() {
										if ( typeof VK !== 'undefined' ) {
											...
											setTimeout(VK_Share_button_$divid,1000);
										};
									};
									VK_Share_button_$divid();
								-->
							</script>";
        }
			...
			<li>
			    <div style="width:130px">
			        <a
                        ...
                        href="' . urldecode($url) . '"
                        data-mrc-config=\'{"nt" : "1", "cm" : "1", "ck" : "1", "sz" : "20", "st" : "1", "tp" : "combo"}\'>Нравится</a>
			</div></li>';
        }
        ...
        function dashboard_widget_function()
        {
			...
			<script type="text/javascript">
				function VK_Widgets_Group() {
					...
					else setTimeout(VK_Widgets_Group,1000);
				}
				VK_Widgets_Group();
			</script>';
        }
        ...
        {
            if (!isset($_COOKIE['vkapi_msg'])) {
                return;
            }
            $temp = unserialize($_COOKIE['vkapi_msg']);
            $type = $temp['type'];
            $msg = $temp['msg'];
            add_action('admin_notices', create_function('', "echo '<div class=\'{$type}\'><p>{$msg}</p></div>';"));
        }


        function notice_error($msg = true)
        {
            if ($msg) {
                return;
            } else {
                setcookie(
                    'vkapi_msg',
                    serialize(array('type' => 'error', 'msg' => $msg)),
                    time() + 3600,
                    COOKIEPATH,
                    COOKIE_DOMAIN,
                    false
        ...
        function notice_notice($msg = true)
        {
            if ($msg) {
                return;
            } else {
                setcookie(
                    'vkapi_msg',
                    serialize(array('type' => 'updated', 'msg' => $msg)),
                    time() + 3600,
                    COOKIEPATH,
                    COOKIE_DOMAIN,
                    false
        ...
        function crosspost_get_image($post_id)
        {
        ...
        function crosspost($vk_at, $post_id, $post_title, $post_content)
        {
            $body = array();
            $body['access_token'] = $vk_at;
            $body['from_group'] = 1;
            $body['signed'] = 1;
            // Group todo-dx: попозжее сделать кросспост в фейсбук, хотя ...
            ...
            if (!is_numeric($vk_group)) {
                ...
                $result = wp_remote_get(self::$vkapi_m . 'groups.getById?' . $query);
                if (is_wp_error($result)) {
                    $msg = $result->get_error_message();
                    self::notice_error('Crosspost: ' . $msg);

                    return false;
                }
                $r_data = json_decode($result['body'], true);
                if (!$r_data['response']) {
                    $msg = $r_data['error']['error_code'];
                    self::notice_error('Crosspost: ' . $msg);
            ...
            // Attachment
            $att = array();
            $image_path = self::crosspost_get_image($post_id);
            if ($image_path) {
                $att[] = self::vk_upload_photo($vk_at, $vk_group, $image_path);
            }
            ...
            if (isset($r_data['response'])) {
                $temp = isset($vk_group_screen_name) ? $vk_group_screen_name : $vk_group;
                $post_link = "http://vk.com/club{$temp}?w=wall-{$vk_group}_{$r_data['response']['post_id']}%2Fall";
                $post_href = "<a href='{$post_link}' target='_blank'>{$r_data['post_title']}</a>";
        ...
        function post_insert($data, $postarr)
        {
            // get ID
            $post_id = $postarr['ID'];
            // check option
            ...
            if ($temp == 'true') {
                self::notice_notice(__('Already crossposted.', self::$plugin_domain));
            ...
            // Start !!!
            self::crosspost($vk_at, $post_id, $data['post_title'], $data['post_content']);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        function add_custom_box($page, $context)
        {
            add_meta_box(
                'vkapi_meta_box_comm',
                __('VKapi: Comments', self::$plugin_domain),
            ...
            add_meta_box(
                'vkapi_meta_box_butt',
                __('VKapi: Social buttons', self::$plugin_domain),
        ...
        {
            global $post;
            echo '<input type="hidden" name="vkapi_noncename" id="vkapi_noncename" value="' . wp_create_nonce(
            ...
            if (empty($conn)) {
                ...
                <td>
                    <div id="vkapi_login_button"
                         ...
                         onclick="VK.Auth.login(onSignonProfile)"><a>ВойтиВКонтакте</a></div>
		...
		<?php
                ...
                <td><p><?php _e('Connected User Id : ', self::$plugin_domain); echo $conn; ?></p>
                    <input type="button"
                           ...
                           value="<?php _e('Disconnect from VKontakte', self::$plugin_domain); ?>"
                           onclick="vkapi_profile_update(1)"/>
            ...
            <div class="misc-pub-section">
                <input
                    ...
                    <?php echo get_option('vkapi_crosspost_default') ? 'checked' : '';?>
                        /> <?php _e('CrossPost to VK.com Wall', self::$plugin_domain); ?>
                <br/>
                <?php _e('Text length:', self::$plugin_domain); ?> <input
                    type="text"
                ...
                <input
                    ...
                    <?php echo get_option('vkapi_crosspost_link') ? 'checked' : '';?>
                        /> <?php _e('Show Post link:', self::$plugin_domain); ?>
            </div><?php } else { ?>
            <div class="misc-pub-section">
        ...
        function vk_upload_photo($vk_at, $vk_group, $image_path)
        {
            ...
            $params['gid'] = $vk_group;
            $query = http_build_query($params);
            $result = wp_remote_get(self::$vkapi_m . 'photos.getWallUploadServer?' . $query);
            if (is_wp_error($result)) {
                $msg = $result->get_error_message();
                self::notice_error('Crosspost: ' . $msg);
            ...
            $data = json_decode($result['body'], true, 1);
            if (!$data['photo']) {
                $msg = $data['error']['error_code'];
                self::notice_error('Crosspost: ' . $msg);
            ...
            $query = http_build_query($params);
            $result = wp_remote_get(self::$vkapi_m . 'photos.saveWallPhoto?' . $query);
            if (is_wp_error($result)) {
                $msg = $data->get_error_message();
                self::notice_error('Crosspost: ' . $msg);
...
if (version_compare($wp_version, "3.4", "<")) {
    function vkapi_notice_update()
    {
        echo '
		<div class="error">
		<p>VKontakte API plugin requires Wordpress 3.4 or newer. <a href="' . bloginfo(
            'url'
        ) . 'wp-admin/update-core.php' . '">Please update!</a></p>
