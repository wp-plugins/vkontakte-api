...
class Punycode
{
    ...
    private static function charcode_from_string($string)
    {
        return (int)preg_replace(
            '/^.*?([0-9]+).*$/',
            '$1',
            mb_convert_encoding($string, 'HTML-ENTITIES', self::$OPTIONS['charset'])
        );
    }
    ...
    public static function _decode($input)
    {
        ...
        while ($pos < $ilen) {
            ...
            $n += (int)($i / $olen);
            $i %= $olen;
            $output = mb_substr($output, 0, $i, self::$OPTIONS['charset']) . self::string_from_charcode($n) . mb_substr(
                $output,
                $i,
                $olen - $i,
    ...
    public static function _encode($input)
    {
        ...
        for ($b = 0; $b < $ilen; ++$b) {
            if (($code = ord($char = mb_substr($input, $b, 1, self::$OPTIONS['charset']))) < $n) {
                ...
                if (!in_array($code = self::charcode_from_string($char), $non_basic_codepoints)) {
